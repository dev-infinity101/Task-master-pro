  ...
  return (
    ...
    >
      <Icon
        className={cn(
          "w-4 h-4 shrink-0",
          active ? "text-white" : "text-[#888888] group-hover:text-[#111111]",
        )}
      />
      ...
      {badge !== undefined && (
        <span
          className={cn(
            ...
            active ? "bg-white/20 text-white" : "bg-[#E5E7EB] text-[#555555]",
          )}
        >
          {badge}
        </span>
...
export default function Dashboard() {
  ...
  // Mobile sidebar toggle  -  hidden by default on small screens
  ...
  const activeTasks = (tasks[activeProjectId] ?? []).filter(
    (t) => t.status !== "done",
  );
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#F7F8FA] dark:bg-background text-[#111111] dark:text-foreground">
      ...
      <aside
        className={cn(
          // Always show on md+; on mobile use fixed overlay
          "flex flex-col border-r border-[#E5E7EB] dark:border-[#1F1F1F] bg-white dark:bg-[#0D0D0D]",
          // Desktop: normal flow, fixed width
          "md:relative md:flex md:w-[260px] md:shrink-0 md:z-auto md:translate-x-0",
          // Mobile: fixed overlay, slide in/out
          "fixed inset-y-0 left-0 z-50 w-[260px] transition-transform duration-300 ease-in-out",
          mobileSidebarOpen
            ? "translate-x-0"
          ...
          <div className="px-4 mb-2">
            ...
            <div className="space-y-0.5">
              ...
              {!projectsLoading && projects.length === 0 && (
                <div className="px-2 py-2 text-xs text-[#9CA3AF]">
                  No projects yet.
                </div>
              )}
              {!projectsLoading &&
                projects.map((project) => {
                  ...
                  const pendingCount = (tasks[project.id] ?? []).filter(
                    (t) => t.status !== "done",
                  ).length;
                  return (
                    <button
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Header */}
        <header className="flex h-14 md:h-16 items-center gap-2 md:gap-4 px-3 md:px-6 border-b border-[#E5E7EB] dark:border-[#1F1F1F] bg-white dark:bg-[#0D0D0D] shrink-0 z-10">
          {/* Mobile sidebar toggle  -  only shown on small screens */}
          <button
            className="md:hidden flex items-center justify-center w-9 h-9 rounded-lg hover:bg-[#F3F4F6] dark:hover:bg-accent transition-colors shrink-0"
            onClick={() => setMobileSidebarOpen(!mobileSidebarOpen)}
            aria-label="Toggle sidebar"
          >
            {mobileSidebarOpen ? (
              <PanelLeftClose className="w-5 h-5 text-[#555555] dark:text-muted-foreground" />
            ) : (
              <PanelLeftOpen className="w-5 h-5 text-[#555555] dark:text-muted-foreground" />
            )}
          </button>
          ...
          <div className="flex items-center bg-[#F3F4F6] dark:bg-secondary p-1 rounded-xl gap-0.5">
            {VIEW_TABS.map((tab) => (
              <button
                ...
                title={tab.label}
                className={cn(
                  "flex items-center gap-1.5 px-2 md:px-3 py-1.5 rounded-md md:rounded-lg text-xs font-semibold transition-all duration-150",
                  dashView === tab.id
          ...
          <div
            className={cn(
              "hidden sm:flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-full font-medium transition-colors shrink-0",
              wsConnected
        ...
        <main className="flex-1 overflow-hidden relative bg-[#F7F8FA] dark:bg-[#0F0F0F]">
          {!activeProjectId ? (
            <div className="flex flex-col items-center justify-center h-full text-center px-4">
              ...
              <h3 className="text-xl font-bold text-[#111111] dark:text-foreground mb-2">
                No project selected
              </h3>
                ...
                <div className="h-full overflow-y-auto">
                  <div className="max-w-2xl mx-auto px-6 py-6 space-y-2">
                    ...
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <div>
                        <h2 className="text-lg font-bold text-[#111111] dark:text-foreground leading-none">
                          AI Intelligence
                        </h2>
                        <p className="text-xs text-[#9CA3AF] dark:text-muted-foreground mt-0.5">
