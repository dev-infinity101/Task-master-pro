/**
 * Roadmap.jsx - Year-Long Roadmap Visualization Page
 *
 * Accessible via:
...
const WEEK_GOAL_KEYS = [
    "week_1_goal",
    "week_2_goal",
    "week_3_goal",
    "week_4_goal",
];
const WEEK_DONE_KEYS = [
    "week_1_done",
    "week_2_done",
    "week_3_done",
    "week_4_done",
...
/* Helper: get week date ranges for a month */
function getWeekRanges(year, month) {
    ...
    for (let w = 0; w < 4; w++) {
        ...
        const fmtStart = new Date(year, month - 1, start).toLocaleDateString(
            "en-US",
            { month: "short", day: "numeric" },
        );
        const fmtEnd = new Date(year, month - 1, end).toLocaleDateString("en-US", {
            month: "short",
            day: "numeric",
        });
        ranges.push(`${fmtStart} - ${fmtEnd}`);
    }
    return ranges;
}

/* MonthCard (Semi-Rounded) */
function MonthCard({
    fwdRef,
    dotRef,
    monthNum,
    ...
    return (
        <div
            ref={fwdRef}
            className={cn(
                "flex-shrink-0 cursor-pointer relative flex flex-col justify-center px-4 md:px-8",
                // Size & Semi-Rounded Shape
                "w-[85vw] md:w-[420px] h-[72px] md:h-[86px] rounded-2xl",
                ...
                isActive &&
                "border-[#22c55e] dark:border-[#22c55e] ring-2 ring-[#22c55e]/30",
            )}
            onClick={onClick}
        >
            <div className="flex items-center justify-between pointer-events-none">
                <div className="flex items-center gap-3.5">
                    ...
                    <div
                        ...
                        style={
                            hasGoals || isActive
                                ? { backgroundColor: isActive ? "#22c55e" : accentColor }
                                : {}
                        }
                    />
                    <h2
                        className={cn(
                            "text-[13px] md:text-[15px] font-black tracking-widest uppercase mt-[0.5px]",
                            isActive || hasGoals
                                ? "text-foreground"
            ...
            {superGoal.trim() && (
                <div className="mt-1 pl-6 overflow-hidden pointer-events-none">
                    <p className="text-[12.5px] md:text-[14.5px] text-[#3b82f6] dark:text-blue-400 font-extrabold italic truncate animate-neon-pulse">
                        {"\u2726"} {superGoal}
                    </p>
                </div>
            )}
                    ...
                    <div className="p-6 space-y-6 flex-1 overflow-y-auto">
                        ...
                        <section>
                            ...
                            <p className="text-xs text-muted-foreground mb-2">
                                The one thing that defines this month's success.
                            </p>
                            <textarea
                        ...
                        <section>
                            ...
                            <textarea
                                ...
                                rows={4}
                                placeholder="Brain dump, planning notes, context..."
                                className={cn(
                                    "w-full bg-muted/50 border border-border rounded-xl px-4 py-3",
                                    "text-sm text-foreground placeholder:text-muted-foreground",
...
export default function Roadmap() {
    ...
    const [scrollPct, setScrollPct] = useState(0);
    // Mobile sidebar  -  must be declared before any conditional returns
    const [mobileSidebarOpen, setMobileSidebarOpen] = useState(false);

    const { months, upsertMonth } = useRoadmap(activeProjectId, year);

    // Build a lookup: month -> data row
    ...
    /* Scroll handler (Custom Progress Bar + Snap Detection) */
    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            if (targetRef)
                targetRef.scrollIntoView({ block: "center", behavior: "instant" });
        }, 50);
    }, [currentMonth, cardRefs]);

    /* Navigate between months using sidebar or nav arrows */
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-background text-foreground relative z-0">
            ...
            <aside
                className={cn(
                    "flex flex-col border-r border-border bg-card/80 backdrop-blur-xl z-50",
                    // Desktop: always visible, normal flow
                    "md:relative md:flex md:w-[220px] md:shrink-0 md:translate-x-0 md:z-20",
                    // Mobile: fixed overlay, slide in/out
                    "fixed inset-y-0 left-0 w-[220px] transition-transform duration-300 ease-in-out",
                    mobileSidebarOpen
                        ? "translate-x-0"
                ...
                <div className="flex-1 overflow-y-auto py-3 px-3">
                    ...
                    {MONTH_NAMES.map((name, i) => {
                        ...
                        const hasGoal = Boolean(
                            data?.super_goal || WEEK_GOAL_KEYS.some((k) => data?.[k]),
                        );
            ...
            <div className="flex-1 flex flex-col overflow-hidden z-10">
                {/* Top nav bar */}
                <header className="flex h-14 md:h-16 items-center justify-between px-3 md:px-8 border-b border-border bg-card/80 backdrop-blur-xl shrink-0 z-20">
                    ...
                    <div className="flex items-center gap-2 md:gap-3">
                        <button
                            ...
                            className="flex items-center gap-1.5 text-xs px-2 md:px-3 py-1.5 md:py-2 rounded-lg bg-muted hover:bg-accent text-foreground transition-colors"
                        >
                            <Calendar className="w-3.5 h-3.5" />
                            <span className="hidden sm:inline">Today</span>
                        </button>
                        <div className="text-[10px] md:text-xs text-muted-foreground w-20 md:w-28 text-right font-medium">
                        ...
                        */}
                        <div className="relative flex flex-col items-center w-full pb-[35vh] pt-[35vh]">
                            ...
                            {Array.from({ length: 12 }, (_, i) => {
                                ...
                                const data = monthData[monthNum];
                                const isActive =
                                    monthNum === currentMonth && year === currentYear;

                                // Zigzag staggered layout: odd/even shifting left & right
                                // Reduce stagger on mobile so cards don't clip off-screen
                                const isLeft = i % 2 === 0;
                                const isMobile =
                                ...
                                return (
                                    ...
                                    >
                                        <div style={{ transform: `translateX(${shiftX}px)` }}>
                                            <MonthCard
                                                fwdRef={(el) => {
                                                    if (cardRefs[i]) cardRefs[i].current = el;
                                                }}
                                                dotRef={(el) => {
                                                    if (dotRefs[i]) dotRefs[i].current = el;
                                                }}
                                                monthNum={monthNum}
                                                year={year}
                                                data={data}
