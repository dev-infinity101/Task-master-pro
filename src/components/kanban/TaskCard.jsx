...
import { CheckSquare, Flag, Trash2, Clock, AlertTriangle } from "lucide-react";
    ...
    <DropdownMenu>
      ...
      >
        ...
        <DropdownMenuRadioGroup
          value={currentKey}
          onValueChange={(value) => {
            updateTask(task.id, task.project_id ?? activeProjectId, {
              priority: value,
            });
          }}
        >
...
export default function TaskCard({ task, isDragging = false }) {
  ...
  // Urgency from deadline  -  always 'none' for completed tasks (memoised)
  ...
  // Done tasks: no urgency ring, bg, or coloured left-border ever
  const urgencyRing = isDone
    ? ""
    : ({
        none: "",
        normal: "",
  ...
  return (
    ...
    >
      ...
      >
        <CardContent className="p-2.5 md:p-3.5 space-y-1.5 md:space-y-2.5">
          {/*  Header  */}
          ...
          <div className="flex items-center justify-between gap-1.5 md:gap-2 pt-0.5">
            {/* Left cluster */}
            <div className="flex items-center gap-1.5 md:gap-2">
              ...
              {/* Priority badge (only when no urgency override, and task is not done) */}
              {!isDone &&
                currentPriorityKey !== "none" &&
                (urgency === "none" || urgency === "normal") && (
                  <div
